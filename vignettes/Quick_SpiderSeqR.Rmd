...
### Search more directories to locate the database files


By default, startSpiderSeqR() tries to locate the database files within 
the specified directory. If files are not present there, 
it will increase the scope to a **recurse_levels** number of directories above
(and all their subdirectories).

**TASK:** *"I would like to be more flexible about the location* 
*of the database files"*

To increase the scope of the search, set **recurse_levels** to a higher level.
Please note, however, that if the number is too large 
(e.g. large enough to reach your root directory), the search might take 
much longer time.


```{r, eval = FALSE}
startSpiderSeqR(getwd(), recurse_levels = 4)
```
