...
filterByTerm <- function(df, query, filter_columns=NULL){
    ...
    .GlobalEnv$filter_con <- DBI::dbConnect(RSQLite::SQLite(), dbname = filter_db_file)
    #RSQLite::dbWriteTable(conn = get("filter_con", envir = .GlobalEnv), name="df", value = df_subset, field.types = NULL, row.names = FALSE, append = TRUE)
    RSQLite::dbWriteTable(conn = get("filter_con", envir = .GlobalEnv), name="df", value = df_subset)
