...
#' 
#' @param acc_vector A character vector with the accessions to be found within data frame
#' @param df Data frame to be checked (needs to contain at least the column corresponding to accession type that is searched for)
#' @param output A character indicating the type of output, can be \code{"logical"} (returns \code{TRUE} if all (non-NA, non-empty, unique) acc_vector elements are present in the data frame) or \code{"numeric"} (returns a numerical vector with the count of matches and the length of (non-NA, non-empty, unique) acc_vector)
...
countAccessionMatches <- function(acc_vector, df, output = "logical"){
  ...
  # Get the vector with accessions from the df (special treatment for series_id) ####
  ...
  if (match_count == length(acc_vector)){
    mm(paste0("All accessions found (", match_count, "/", length(acc_vector), ")."), "res")
    
  } else {
    mm(paste0("Not all accessions found (", match_count, "/", length(acc_vector), ")."), "res")
    if (length(missing_acc)<200){ # Do not overflow the console output with lots of accessions
      mm(paste0("Missing accessions: ", paste0(missing_acc, collapse = ", ")), "res")
    }
  }
